...
#
# В этом скрипте проводится серия экспериментов для разных значений
# интенсивности атак `λ`, после чего сравниваются эмпирические и
# теоретические вероятности события `N > 10`.
...
#
# Зададим общие параметры моделирования и набор значений интенсивности
# атак `λ`, для которых будет выполнена серия экспериментов.
...
#
# Для каждого значения `λ` моделируется поток атак, оценивается вероятность
# события `N > 10`, а также сохраняются детальные графики.
...
# ## Сводная таблица
#
# После выполнения всех экспериментов сохраним сводную таблицу с результатами
# в форматах CSV и JLD2.
...
# ## Обобщающий график
#
# Построим зависимость вероятности события `N > 10` от интенсивности атак `λ`.
